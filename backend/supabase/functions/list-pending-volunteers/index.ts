...
export async function handler(req: Request): Promise<Response> {
  ...
  // Read-only queue listing: a generous ceiling for a staff UI that polls.
  const allowed = await checkRateLimit(supabase, `pending-vols:${ip}`, 300, 3600);
  if (!allowed) {
  ...
  try {
    ...
    const result = await listPendingVolunteers(supabase, claims, input);
